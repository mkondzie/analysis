...
void calibrationSi() {
  ...
  for (const auto &l : workingDetectors) {
    ...
    gr[l]->Fit(fChVeryk[l],"RME0","",500,4000);
    ...
    K[l] = energyAlphaMeV /
           (a + b * xAlphaPeakChannel + c * TMath::Power(xAlphaPeakChannel, 2) +
            d * TMath::Power(xAlphaPeakChannel, 2.5) +
            e * TMath::Power(xAlphaPeakChannel, 3));
  }
  ...
  for (const auto &l : workingDetectors) {
    ...
    if (l != 7) {
      Double_t NeAuEnergyMeV =
          K[l] * (A[l] + B[l] * NeAuChannel.at(l) +
                  C[l] * TMath::Power(NeAuChannel.at(l), 2) +
                  D[l] * TMath::Power(NeAuChannel.at(l), 2.5) +
                  E[l] * TMath::Power(NeAuChannel.at(l), 3));
  ...
  for (auto &l : workingDetectors) {
    ...
    for (Long64_t i = 0; i < cc21->GetEntries(); i++) {
      cc21->GetEntry(i);
      if (detectorN == l) {
        energyMeV = K[l] * (A[l] + B[l] * energyChannel +
                            C[l] * TMath::Power(energyChannel, 2) +
                            D[l] * TMath::Power(energyChannel, 2.5) +
                            E[l] * TMath::Power(energyChannel, 3));
